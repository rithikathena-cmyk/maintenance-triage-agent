"""Agentic triage — a real Claude tool-use loop.

Unlike ``claude_service`` (one structured call per order, no tools), here Claude
is an autonomous agent: it is given tools and DRIVES the flow itself in a loop —

    read_queue  (a real MCP tool)  -> Claude fetches the pending orders
    submit_triage (an action tool) -> Claude records its decision per order
    ... Claude keeps calling tools until every order is triaged, then stops.

The backend only executes the tools Claude asks for and feeds results back.

SAFETY: Claude is handed ONLY read/propose tools. The ``write_assignment`` MCP
tool is never in its toolset, so the agent structurally cannot assign work —
``submit_triage`` writes a *proposal* (still requires human approval), and the
deterministic safety guard is re-applied on top of whatever the agent proposes.
"""
import json
import os

from sqlalchemy.orm import Session

from backend.database import models
from backend.services import assignment_service, triage_service
from backend.services.mcp_client import QUEUE_SERVER, run_tool
from backend.services.safety_rules import CREWS, URGENCY_LEVELS

MODEL = os.getenv("CLAUDE_MODEL", "claude-opus-4-8")
_MAX_TURNS = 16  # hard stop so a misbehaving loop can never run away
_MAX_DISPATCH_TURNS = 40  # the full dispatcher does more per queue (triage + assign)

AGENT_SYSTEM = f"""You are an autonomous maintenance-triage agent for a machine
shop / production floor. You have tools and you drive the triage yourself.

Work in this loop, orchestrating your tools:
1. Call `read_queue` (status="pending") to fetch the work orders waiting for triage.
2. Optionally call `get_crew_load` to see how many orders are already queued per crew,
   so you can balance workload when two crews could both handle an order.
3. For EACH order returned, decide its urgency and the right technician crew, then
   call `submit_triage` with your decision — exactly one call per order.
4. Once you have submitted a triage for every order you read, stop and reply with a
   one-line summary. Do not call read_queue again in a loop.

Urgency is one of: {", ".join(URGENCY_LEVELS)}.
  - "safety-critical": anyone could be hurt — pinch points, exposed moving parts,
    hydraulic bursts, arc flash, missing guards, injury reports.
  - "production-stopping": the machine is down or unsafe to run (halting output),
    but no immediate injury risk.
  - "routine": degraded or cosmetic; production continues.
Crew is one of: {", ".join(CREWS)}.

Set is_safety_risk=true whenever anyone could be injured. Give a one/two sentence
reasoning and a confidence from 0.0 to 1.0 (use the full range). You only PROPOSE —
a human dispatcher approves every assignment. Never try to assign work yourself."""

TOOLS = [
    {
        "name": "read_queue",
        "description": "Read work orders from the maintenance queue by lifecycle status.",
        "input_schema": {
            "type": "object",
            "properties": {
                "status": {
                    "type": "string",
                    "enum": ["pending", "triaged", "assigned", "rejected"],
                    "description": "Which orders to read. Use 'pending' for untriaged ones.",
                },
            },
            "required": ["status"],
        },
    },
    {
        "name": "get_crew_load",
        "description": "How many orders are currently awaiting review per crew — use it to balance workload.",
        "input_schema": {"type": "object", "properties": {}},
    },
    {
        "name": "submit_triage",
        "description": "Record your triage decision for ONE work order (creates a proposal, not an assignment).",
        "input_schema": {
            "type": "object",
            "properties": {
                "work_order_id": {"type": "integer"},
                "urgency": {"type": "string", "enum": URGENCY_LEVELS},
                "crew": {"type": "string", "enum": CREWS},
                "is_safety_risk": {"type": "boolean"},
                "reasoning": {"type": "string"},
                "confidence": {"type": "number"},
            },
            "required": ["work_order_id", "urgency", "crew", "is_safety_risk", "reasoning", "confidence"],
        },
    },
]


def _exec_read_queue(tool_input: dict, limit: int) -> str:
    """Run the REAL read_queue MCP tool; cap the batch the agent sees at `limit`."""
    status = tool_input.get("status", "pending")
    rows = run_tool(QUEUE_SERVER, "read_queue", {"status": status, "limit": limit})
    return json.dumps(rows)


def _exec_crew_load(db: Session) -> str:
    """Current count of awaiting-review proposals per crew (for load balancing)."""
    from sqlalchemy import func

    db.rollback()  # fresh snapshot so the agent's own just-submitted rows count
    rows = (
        db.query(models.Proposal.proposed_crew, func.count())
        .join(models.WorkOrder)
        .filter(models.WorkOrder.status == models.STATUS_TRIAGED)
        .group_by(models.Proposal.proposed_crew)
        .all()
    )
    load = {crew: 0 for crew in CREWS}
    for crew, n in rows:
        load[crew] = n
    return json.dumps(load)


def _exec_submit_triage(db: Session, tool_input: dict, submitted: set) -> str:
    """Apply the safety guard and upsert the agent's proposal for one order."""
    wo_id = tool_input.get("work_order_id")
    work_order = db.get(models.WorkOrder, wo_id) if wo_id is not None else None
    if work_order is None:
        return json.dumps({"ok": False, "error": f"work order {wo_id} not found"})
    if work_order.status != models.STATUS_PENDING:
        return json.dumps({"ok": False, "error": f"work order {wo_id} is not pending"})

    conf = tool_input.get("confidence")
    if isinstance(conf, (int, float)):
        conf = max(0.0, min(1.0, float(conf)))
    # source="agent" marks this as coming from the tool-use loop, not a single call.
    proposal_data = {
        "urgency": tool_input["urgency"],
        "crew": tool_input["crew"],
        "reasoning": tool_input.get("reasoning"),
        "confidence": conf,
        "source": "agent",
    }
    triage_service._upsert_proposal(db, work_order, proposal_data)
    db.commit()  # persist per order — the deterministic safety guard ran inside
    submitted.add(wo_id)
    return json.dumps({"ok": True, "work_order_id": wo_id})


def agentic_triage(db: Session, limit: int = 8) -> dict:
    """Triage pending orders with a genuine Claude tool-use loop.

    Returns the same shape as ``triage_service.run_triage`` so the frontend can
    drive it with the identical chunked-progress loop.
    """
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        # No key -> no agent; fall back to the deterministic path transparently.
        return triage_service.run_triage(db, limit=limit)

    # Reuse the same single-run lock so agentic and classic runs never overlap.
    if not triage_service._triage_lock.acquire(blocking=False):
        return {"triaged": 0, "queue_size": 0, "remaining": triage_service._pending_count(db), "busy": True}

    try:
        import anthropic

        client = anthropic.Anthropic(api_key=api_key)
        messages = [{"role": "user", "content": f"Triage the pending queue now (up to {limit} orders)."}]
        submitted: set = set()

        for _turn in range(_MAX_TURNS):
            response = client.messages.create(
                model=MODEL,
                max_tokens=4096,
                system=AGENT_SYSTEM,
                tools=TOOLS,
                messages=messages,
            )
            messages.append({"role": "assistant", "content": response.content})
            if response.stop_reason != "tool_use":
                break  # agent finished (no more tool calls)

            tool_results = []
            for block in response.content:
                if getattr(block, "type", None) != "tool_use":
                    continue
                if block.name == "read_queue":
                    content = _exec_read_queue(block.input, limit)
                elif block.name == "get_crew_load":
                    content = _exec_crew_load(db)
                elif block.name == "submit_triage":
                    content = _exec_submit_triage(db, block.input, submitted)
                else:
                    content = json.dumps({"ok": False, "error": f"unknown tool {block.name}"})
                tool_results.append(
                    {"type": "tool_result", "tool_use_id": block.id, "content": content}
                )
            messages.append({"role": "user", "content": tool_results})

        return {
            "triaged": len(submitted),
            "queue_size": limit,
            "remaining": triage_service._pending_count(db),
            "busy": False,
        }
    except Exception:
        # Never let an agent-loop error fail the request; the classic path is safe.
        db.rollback()
        return triage_service.run_triage(db, limit=limit)
    finally:
        triage_service._triage_lock.release()


# =========================================================================== #
# FULL AGENTIC DISPATCHER
#
# Unlike ``agentic_triage`` above (propose-only), this agent drives the ENTIRE
# dispatch end to end: it reads the queue, triages, and then actually ASSIGNS
# each order to a crew (or rejects it) through the write_assignment MCP tool —
# no human button in the loop. It runs as a generator that yields events so the
# UI can stream Claude's tool activity live as it works.
# =========================================================================== #
DISPATCH_SYSTEM = f"""You are the autonomous dispatcher for a machine shop /
production floor. You OWN the maintenance queue and drive it to zero yourself,
using your tools — there is no human approving your decisions.

Run this loop until the queue is empty, then stop:
1. Call `read_queue` with status="pending" and status="triaged" to see all
   outstanding work. Pending orders still need triage; triaged orders already
   have a proposal and just need dispatching.
2. Optionally call `get_crew_load` to balance workload across crews.
3. For each PENDING order: call `submit_triage` (records urgency + crew), then
   `assign_order` to commit it to a crew — or `reject_order` if it is spam /
   not actionable.
4. For each already-TRIAGED order: call `assign_order` (or `reject_order`).
5. Batch several tool calls in one turn when you can — it is faster.
6. When both queues are empty, reply with a one-line summary and stop. Do not
   keep calling read_queue in an empty loop.

Urgency is one of: {", ".join(URGENCY_LEVELS)}.
  - "safety-critical": anyone could be hurt — pinch points, exposed moving
    parts, hydraulic bursts, arc flash, missing guards, injury reports.
  - "production-stopping": the machine is down or unsafe to run, but no
    immediate injury risk.
  - "routine": degraded or cosmetic; production continues.
Crew is one of: {", ".join(CREWS)}.

Set is_safety_risk=true whenever anyone could be injured; the system also runs a
deterministic safety guard on top of your call, so a hazard can never be
downgraded. Give a one/two sentence reasoning and a confidence 0.0–1.0. Assign
safety-critical work to "Safety / Hazmat" unless another crew is clearly the
right fit for the actual repair. You have full authority — dispatch decisively."""

DISPATCH_TOOLS = [
    TOOLS[0],  # read_queue
    TOOLS[1],  # get_crew_load
    TOOLS[2],  # submit_triage
    {
        "name": "assign_order",
        "description": (
            "Commit a work order to a crew — this WRITES the assignment via the "
            "assignment MCP tool. The order must already be triaged (call "
            "submit_triage first for pending orders)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "work_order_id": {"type": "integer"},
                "crew": {"type": "string", "enum": CREWS},
            },
            "required": ["work_order_id", "crew"],
        },
    },
    {
        "name": "reject_order",
        "description": "Reject a work order (spam / not actionable). No assignment is written.",
        "input_schema": {
            "type": "object",
            "properties": {
                "work_order_id": {"type": "integer"},
                "reason": {"type": "string"},
            },
            "required": ["work_order_id"],
        },
    },
]


def _exec_assign(db: Session, tool_input: dict, actor: str, counts: dict):
    """Approve+assign one order through the write_assignment MCP path."""
    wo_id = tool_input.get("work_order_id")
    crew = tool_input.get("crew")
    try:
        a = assignment_service.approve(
            db, work_order_id=wo_id, approved_by=actor, crew_override=crew
        )
        counts["assigned"] += 1
        content = json.dumps({"ok": True, "work_order_id": wo_id, "assignment_id": a.id, "crew": a.crew})
        return content, {"type": "tool_result", "name": "assign_order", "ok": True,
                         "summary": f"WO-{wo_id} → {a.crew}", "work_order_id": wo_id}
    except Exception as exc:
        db.rollback()
        content = json.dumps({"ok": False, "work_order_id": wo_id, "error": str(exc)})
        return content, {"type": "tool_result", "name": "assign_order", "ok": False,
                         "summary": f"WO-{wo_id} not assigned: {exc}", "work_order_id": wo_id}


def _exec_reject(db: Session, tool_input: dict, actor: str, counts: dict):
    """Reject one order (marks it rejected; writes no assignment)."""
    wo_id = tool_input.get("work_order_id")
    try:
        assignment_service.reject(
            db, work_order_id=wo_id, rejected_by=actor, reason=tool_input.get("reason")
        )
        counts["rejected"] += 1
        content = json.dumps({"ok": True, "work_order_id": wo_id})
        return content, {"type": "tool_result", "name": "reject_order", "ok": True,
                         "summary": f"WO-{wo_id} rejected", "work_order_id": wo_id}
    except Exception as exc:
        db.rollback()
        content = json.dumps({"ok": False, "work_order_id": wo_id, "error": str(exc)})
        return content, {"type": "tool_result", "name": "reject_order", "ok": False,
                         "summary": f"WO-{wo_id} reject failed: {exc}", "work_order_id": wo_id}


def _dispatch_exec(db: Session, block, actor: str, counts: dict):
    """Run one tool the dispatcher asked for; return (content_for_model, ui_event)."""
    name = block.name
    inp = block.input or {}
    if name == "read_queue":
        status = inp.get("status", "pending")
        rows = run_tool(QUEUE_SERVER, "read_queue", {"status": status, "limit": 100})
        return json.dumps(rows), {"type": "tool_result", "name": name, "ok": True,
                                  "summary": f"{len(rows)} {status} order(s)"}
    if name == "get_crew_load":
        return _exec_crew_load(db), {"type": "tool_result", "name": name, "ok": True,
                                     "summary": "read crew load"}
    if name == "submit_triage":
        content = _exec_submit_triage(db, inp, set())
        ok = json.loads(content).get("ok", False)
        if ok:
            counts["triaged"] += 1
        wo_id = inp.get("work_order_id")
        summary = (f"WO-{wo_id}: {inp.get('urgency')} · {inp.get('crew')}"
                   if ok else f"WO-{wo_id}: {json.loads(content).get('error')}")
        return content, {"type": "tool_result", "name": name, "ok": ok,
                         "summary": summary, "work_order_id": wo_id}
    if name == "assign_order":
        return _exec_assign(db, inp, actor, counts)
    if name == "reject_order":
        return _exec_reject(db, inp, actor, counts)
    return (json.dumps({"ok": False, "error": f"unknown tool {name}"}),
            {"type": "tool_result", "name": name, "ok": False, "summary": "unknown tool"})


def agentic_dispatch(db: Session, actor: str = "Claude agent", limit: int = 25):
    """Autonomously work the whole queue with a Claude tool-use loop.

    A GENERATOR: yields event dicts as Claude drives its tools, so the UI can
    stream activity live. Event types:
      start | text | tool_call | tool_result | done | busy | error
    Claude reads the queue, triages, and assigns/rejects every order itself —
    the write path runs unattended here (no human approval button).
    """
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        yield {"type": "error", "error": "No ANTHROPIC_API_KEY — the autonomous dispatcher needs Claude."}
        return

    # Reuse the single-run lock so the dispatcher and any triage run never race.
    if not triage_service._triage_lock.acquire(blocking=False):
        yield {"type": "busy"}
        return

    counts = {"triaged": 0, "assigned": 0, "rejected": 0}
    try:
        import anthropic

        client = anthropic.Anthropic(api_key=api_key)
        messages = [{"role": "user", "content": (
            f"Work the maintenance queue now. Triage every pending order and dispatch "
            f"every order (pending and already-triaged) to the right crew — or reject it. "
            f"Handle up to {limit} orders."
        )}]
        yield {"type": "start"}

        for _turn in range(_MAX_DISPATCH_TURNS):
            response = client.messages.create(
                model=MODEL,
                max_tokens=4096,
                system=DISPATCH_SYSTEM,
                tools=DISPATCH_TOOLS,
                messages=messages,
            )
            messages.append({"role": "assistant", "content": response.content})

            for block in response.content:
                if getattr(block, "type", None) == "text" and block.text.strip():
                    yield {"type": "text", "text": block.text.strip()}

            if response.stop_reason != "tool_use":
                break  # dispatcher finished — no more tool calls

            tool_results = []
            for block in response.content:
                if getattr(block, "type", None) != "tool_use":
                    continue
                yield {"type": "tool_call", "name": block.name, "input": block.input}
                content, event = _dispatch_exec(db, block, actor, counts)
                yield event
                tool_results.append(
                    {"type": "tool_result", "tool_use_id": block.id, "content": content}
                )
            messages.append({"role": "user", "content": tool_results})

        yield {"type": "done", "counts": counts}
    except Exception as exc:
        db.rollback()
        yield {"type": "error", "error": f"{exc.__class__.__name__}: {exc}", "counts": counts}
    finally:
        triage_service._triage_lock.release()
